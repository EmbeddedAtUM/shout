...
public class ClickableLinksTextView extends TextView {
	...
	public void onStartTemporaryDetach() {
		...
		// listview scrolling behaves incorrectly after you select and copy some
		// text, so I've added this code
		if (this.isFocused()) {
			this.clearFocus();
		}
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		// the base TextView class checks if getAutoLinkMask != 0, so I added a
		// similar code for == 0
	...
	private void copyBaseEditorIfNecessary() {
		...
		try {
			...
			if (this.mBaseEditor != null) {
				...
				this.mDiscardNextActionUpField = editorClass
						.getDeclaredField("mDiscardNextActionUp");
				this.mDiscardNextActionUpField.setAccessible(true);

				this.mIgnoreActionUpEventField = editorClass
						.getDeclaredField("mIgnoreActionUpEvent");
