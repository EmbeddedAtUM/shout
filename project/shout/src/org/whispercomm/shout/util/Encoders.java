...
public class Encoders {
	...
	/**
	 * When given a byte array, return the hex-encoding of the byte array.
	 * Indices with lower values are encoded in positions of higher
	 * significance, e.g. the array is read low to high and the string is built
	 * left to right.
	 * 
	 * @param arr
	 * @return Hex-encoding of {@code arr}
	 */
	public static String toHexString(byte[] arr) {
		StringBuilder builder = new StringBuilder(arr.length * 2);
		for (byte b : arr) {
			builder.append(hexArray[MASK & b]);
		}
